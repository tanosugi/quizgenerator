...
export default function UpdateQuizSetForm(props) {
  const {
    id: idProp,
    quizSet: quizSetModelProp,
    onSuccess,
    onError,
    onSubmit,
    onCancel,
  ...
  const resetStateValues = () => {
    const cleanValues = quizSetRecord
      ? { ...initialValues, ...quizSetRecord }
      : initialValues;
    setSubject(cleanValues.subject);
    setLevel(cleanValues.level);
    setNumberOfQuiz(cleanValues.numberOfQuiz);
    setNumberOfAttempt(cleanValues.numberOfAttempt);
    setLng(cleanValues.lng);
    setErrors({});
  };
  const [quizSetRecord, setQuizSetRecord] = React.useState(quizSetModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(QuizSet, idProp)
        : quizSetModelProp;
      setQuizSetRecord(record);
    };
    queryData();
  }, [idProp, quizSetModelProp]);
  React.useEffect(resetStateValues, [quizSetRecord]);
  const validations = {
    subject: [],
    level: [],
    numberOfQuiz: [
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            QuizSet.copyOf(quizSetRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "UpdateQuizSetForm")}
      {...rest}
    >
      <TextField
        label="Subject"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || quizSetModelProp)}
